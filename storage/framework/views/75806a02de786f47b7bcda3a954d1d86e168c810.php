<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="">
    <link rel="icon" type="image/png" href="<?php echo e(asset("public/favicon.ico")); ?>" />

    <title>إدارة التوظيف <?php if(@$title): ?>| <?php echo e(@$title); ?><?php endif; ?></title>

    <!-- Bootstrap Core CSS -->
    <link href="<?php echo e(asset("public/css/rtl/bootstrap.min.css")); ?>" rel="stylesheet">

    <!-- not use this in ltr -->
    <link href="<?php echo e(asset("public/css/rtl/bootstrap.rtl.css")); ?>" rel="stylesheet">

    <!-- MetisMenu CSS -->
    <link href="<?php echo e(asset("public/css/plugins/metisMenu/metisMenu.min.css")); ?>" rel="stylesheet">

    <!-- DataTables CSS -->
    <link href="<?php echo e(asset("public/css/plugins/dataTables.bootstrap.css")); ?>" rel="stylesheet">

    <!-- Custom CSS -->
    <link href="<?php echo e(asset("public/css/rtl/sb-admin-2.css")); ?>" rel="stylesheet">

    <!-- Custom Fonts -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" />
    <link href="<?php echo e(asset("public/css/custom.css")); ?>" rel="stylesheet" type="text/css">

    <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
    <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
    <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->

</head>